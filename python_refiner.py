...
output = open("even_newer_scraping_output.csv", "wb")
writer = UnicodeWriter(output)		
writer.writerow(["Article Title", "Year", "Bytes of Edit","latitude", "longitude"])
with open('new_scraping_output.csv', 'rb') as csvfile:
  reader = csv.reader(csvfile, delimiter=',')
  for row in reader:
    print "foober\n"
    split_row = row[1].split()[0]
    if split_row != 'Date':	
      ...
      writer.writerow(["Python Programming Language", year, real_bytes, row[5], row[6]])	  
output.close()
