...
output = open("divided_sector_info.csv", "wb")
...
def find_placement(input_thing):
    if float(input_thing) < 1.22579:
        return 1
    if 1.22579 <= float(input_thing) and float(input_thing) < 1.702604:
        return 2
    if 1.702604 <= float(input_thing) and float(input_thing) < 2.179419:
        return 3
    if 2.179419 <= float(input_thing):
        return 4
#############################
...
for line in input2:
    if line[1] in industry_dict:
        industry_dict[line[1]][find_placement(line[3])] += 1
        print line[3]
