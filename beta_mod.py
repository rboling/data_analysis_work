...
input2 = csv.reader(open("beta_output.csv"))
class UnicodeWriter:
    """
...
output = open("beta_sector_info.csv", "wb")
...
def find_placement(input_thing):
    if float(input_thing) < .764889:
        return 1
    if .764889 <= float(input_thing) and float(input_thing) < .970589:
        return 2
    if .970589 <= float(input_thing) and float(input_thing) < 1.169315:
        return 3
    if 1.169315 <= float(input_thing):
        return 4
#############################
